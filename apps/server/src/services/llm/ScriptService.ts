// =========================================================================
// ScriptService — 调用 GPT 生成结构化视频脚本
// 复用 Phase 0 verify-1 的 prompt 和 API 逻辑
// =========================================================================
...
export async function generateScript(userPrompt: string): Promise<GptScriptOutput> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`GPT API call failed (${response.status}): ${body}`);
  }
  ...
  // zod 校验
  const result = gptScriptOutputSchema.safeParse(script);
  if (!result.success) {
    throw new Error(
      `Script validation failed: ${result.error.issues.map((i) => `${i.path.join(".")}: ${i.message}`).join("; ")}`
    );
  }
